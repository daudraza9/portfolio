<!DOCTYPE html>
<html lang="en">


<head>
    <!-- Basic SEO -->
    <meta charset="gb18030">

    <!-- Meta -->
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1">

    <meta name="description" content="BugBear deals with the Web Development , Web Designing , SEO and other web based projects with 100% accuracy and assurance of standing first in the market.">
    <meta name="keywords" content="Web Development, BugBear, Web Design, Full Stack Developer, Website, Html5, CSS, JavaScript, Bootstrap, Buggbear, Bugbear, Art of Creation, Front-End Developer, Front-End Design, HTML5, CSS3, JQuery, Bootstrap, Web Design, Web Development, Responsive website, Creative website ">
    <meta name="author" content="Mohsin Malik">
    <meta name="description" content="Creative website development with html5, css3, jQuery & bootstrap framework">
    <meta name="keywords" content="HTML5, CSS3, JQuery, Bootstrap, Web Design, Web Development, Responsive website, Creative website">

    <!-- Title -->
    <title>BuggBear - Mohsin Malik</title>

    <!-- Favicon -->
    <link rel="shortcut icon" href="{{asset('front-assets/img/favicon.png')}}">


    <link href="https://fonts.googleapis.com/css?family=Poppins:400,400i,500,500i,600,600i,700,700i,800,800i,900,900i" rel="stylesheet">
    <link href="https://fonts.googleapis.com/css?family=Open+Sans:300,400,400i,600,600i,700" rel="stylesheet">

    <!-- Template CSS Files -->
    <link href="{{asset('front-assets/css/bootstrap.min.css')}}" rel="stylesheet">
    <link href="{{asset('front-assets/css/preloader.min.css')}}" rel="stylesheet">
    <link href="{{asset('front-assets/css/circle.css')}}" rel="stylesheet">
    <link href="{{asset('front-assets/css/font-awesome.min.css')}}" rel="stylesheet">
    <link href="{{asset('front-assets/css/fm.revealator.jquery.min.css')}}" rel="stylesheet">
    <link href="{{asset('front-assets/css/style.css')}}" rel="stylesheet">

    <!-- CSS Skin File -->
    <link href="{{asset('front-assets/css/skins/yellow.css')}}" rel="stylesheet">

    <link rel="alternate stylesheet" type="text/css" title="yellow" href="{{asset('front-assets/css/skins/yellow.css')}}" />
    <link rel="stylesheet" type="text/css" href="{{asset('front-assets/css/styleswitcher.css')}}" />

    <!-- Modernizr JS File -->
    <script src="{{asset('front-assets/js/modernizr.custom.js')}}"></script>
</head>
